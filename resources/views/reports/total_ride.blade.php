...
<div class="expt_btn pull-right">
 <a href="{{Request::fullUrl()}}{{$estatus}}" @if(count($total_list)== 0) {{"disabled=disabled"}}@endif class="margin-bottom-20 btn btn-dark bg-red-600 color-white">Download EXCEL</a> <a href="{{Request::fullUrl()}}{{$pestatus}}" @if(count($total_list)== 0) {{"disabled=disabled"}}@endif  class="margin-bottom-20 btn btn-dark bg-red-600 color-white">Download PDF</a>
</div>
</div>
...
<script>
 ...
 AmCharts.makeChart( "vehiecle_ride_details", {
  ...
  "titles": [
	...
	"dataProvider": [ 
						@if(count($total_ride_graph) > 0)
					  @foreach($total_ride_graph as $ride_details)
			  			{
