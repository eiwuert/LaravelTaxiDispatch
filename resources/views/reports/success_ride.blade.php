...
<div class="expt_btn pull-right">
 <a href="{{Request::fullUrl()}}{{$estatus}}" @if(count($success_list)== 0) {{"disabled=disabled"}}@endif class="margin-bottom-20 btn btn-dark bg-red-600 color-white">Download EXCEL</a> <a href="{{Request::fullUrl()}}{{$pestatus}}" @if(count($success_list)== 0) {{"disabled=disabled"}}@endif  class="margin-bottom-20 btn btn-dark bg-red-600 color-white">Download PDF</a>
</div>
</div>
...
<tr valign="top" align="center">
	...
	@if($franchise_id == 0)<th align="left">Company Share</th>@endif
</tr>
<tr>
...
<script>
 ...
 AmCharts.makeChart( "vehiecle_ride_details", {
  ...
  "titles": [
	...
	"dataProvider": [ 
						@if(count($success_ride_graph) > 0)
					  @foreach($success_ride_graph as $ride_details)
			  			{
