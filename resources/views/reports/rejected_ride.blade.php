        ...
        <div class="col-lg-12">
			...
			<div class="form-horizontal" >
                ...
                <div class="form-group margin-bottom-20 col-md-3 pull-right" style=" right: -17px; position: relative;">
          				...
          				<button type="button" class="btn btn-dark bg-grey-400 color-black" onclick="window.parent.location='{{ URL::to('/reject_rides')}}'" >Reset</button>
          		</div> 
...
<div class="expt_btn pull-right">
 <a href="{{Request::fullUrl()}}{{$estatus}}" @if(count($rejected_list)== 0) {{"disabled=disabled"}}@endif class="margin-bottom-20 btn btn-dark bg-red-600 color-white">Download EXCEL</a> <a href="{{Request::fullUrl()}}{{$pestatus}}" @if(count($rejected_list)== 0) {{"disabled=disabled"}}@endif  class="margin-bottom-20 btn btn-dark bg-red-600 color-white">Download PDF</a>
</div>
</div>
...
<script>
 ...
 AmCharts.makeChart( "vehiecle_ride_details", {
  ...
  "titles": [
	...
	"dataProvider": [ 
						@if(count($rejected_ride_graph) > 0)
					  @foreach($rejected_ride_graph as $ride_details)
			  			{
