...
function update_profile(){
	...
	$sql1="select * from wy_customer where fb_id=:fb_id";
	$sql3="update wy_customer set name=:name,email=:email where fb_id=:fb_id";
	try{
	
		$db = getConnection();
		$stmt = $db->prepare($sql1); 
		$stmt->bindParam("fb_id", $register->fb_id);
		$stmt->execute();
...
function resend_otp_user($fb_id){
		$date = date("Y-m-d H:i:s");
		$sql="select * from wy_customer where fb_id='$fb_id'";
		...
		try{
			...
			if($car){
				...
				$sql1="update wy_customer set OTP='$verification_code',verification_status='0', otp_time='$date' where fb_id='$fb_id'";
				$stmt = $db->query($sql1);
				$message   =  "Verification code from Wrydes: $verification_code";
...
function otp_verification(){
	...
	$date=date('Y-m-d H:i:s');
	$sql1="select * from wy_customer where fb_id=:fb_id";
	$sql2="select * from wy_customer where fb_id=:fb_id and OTP=:OTP";
	$sql3="update wy_customer set verification_status='1',OTP='' where fb_id=:fb_id";
	try{
...
function userregister(){
	...
	$register = json_decode($request->getBody());
	$date=date('Y-m-d H:i:s');
	$sql1="insert into wy_customer(mobile,fb_id,registered_by,OTP,otp_time,login_status,last_logintime,created_at,updated_at) values(:mobile,:fb_id,'4',:OTP,:otp_time,'1',:last_logintime,:created_at,:updated_at)";
	$verification_code=rand(1111, 9999);
	try{
		$db=getConnection();
		...
		$stmt->bindParam("last_logintime", $date);
		...
		echo '{"Result":"Success","Status":"User registered successfully","details":'.json_encode($id).'}';
...
function chk_user($fbid){
	$date = date("Y-m-d H:i:s");
	$sql="select * from wy_customer where fb_id='$fbid'";
	try{
		$db=getConnection();
		$stmt = $db->query($sql);
		$ride = $stmt->fetchAll(PDO::FETCH_OBJ);
		if($ride){
			echo '{ "Result": "Success","Status":"Customer found"}';
		}else{
...
function conformbooking(){
	...
	try{
		$db = getConnection();
		$insqry = "select * from wy_customer where fb_id=:fb_id";
		...
		if($register->ride_type=='1'){
					...
					if($driverslist){
						...
						if(!$driverslistdet){
							...
							$sql4="insert into wy_ride(city,reference_id,customer_id,source_location,source_lat,source_lng,car_type,ride_type,date_of_ride,destination_location,destination_lat,destination_lng,created_at,updated_at,booked_through) 
								values('$register->city','$ref_id','$userid','$register->location','$register->lat','$register->lng','$register->car_type','$register->ride_type','$date1','$register->destination_location','$register->destination_lat','$register->destination_lng','$date','$date','4')";
							$stmt = $db->query($sql4);
			...
			$sql4="insert into wy_ride(city,reference_id,customer_id,source_location,source_lat,source_lng,car_type,ride_type,schedule_date,schedule_time,date_of_ride,destination_location,destination_lat,destination_lng,created_at,updated_at,booked_through) 
					values('$register->city','$ref_id','$userid','$register->location','$register->lat','$register->lng','$register->car_type','$register->ride_type','$register->schedule_date','$register->schedule_time','$register->schedule_date','$register->destination_location','$register->destination_lat','$register->destination_lng','$date','$date','4')";
			$stmt = $db->query($sql4);
			$ride_id = $db->lastInsertId();
			echo '{"Result":"Success","Status":"Booking confrimed","tripid":'.json_encode($ride_id).',"ride_type":'.json_encode($register->ride_type).'}';
		}
